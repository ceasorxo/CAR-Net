...
import time
from net.CNN_Net import CNN4,CNN5_8,CNN5,CNN4_1,CNN7_1,CNN11,CNN11_1,VGG8_32, ResNet18_8, DnCNN, CNN4_2
...
def iter_batch_train(data_in,data_out,batch_size,epochs,model_route):
    ...
    for i in range(epochs):
        ...
        if ((epoch+1) % 1) == 0:
            model_route = '../model/model8/onnx/CNN_net4-' + str(epoch+1) + '.pt'
...
if __name__ == "__main__":
    ...
    epochs = 100
    batch_size = 128
    model_route = '../model/model8/CNN_net4.pt'
    ...
    data_in = np.zeros((180000,8,8),dtype=np.float64)
    for i in range(len(data_in_o)):
        data_in[i] = data_in_o[i].reshape(8,8)
    data_in = data_in.astype(np.float64)

    data_out_o = data_loader(file_route_out)
    data_out_o = np.array(data_out_o)
    data_out = np.zeros((180000,8,8),dtype=np.float64)
    for i in range(len(data_out_o)):
        data_out[i] = data_out_o[i].reshape(8,8)
